<script lang="ts">
  ...
  import { bodyMetricGroupOrder } from '$lib/metrics/body';
  import type { DerivedPoint } from '$lib/metrics/derived';
  import BodyMeasurementModal from './BodyMeasurementModal.svelte';
  ...
  function groupRankFor(metricName: string) {
    const categories = getMetricDefinition(metricName).categories || [];
    const categoryOrder = ['anthropometry', 'body-index', 'body-composition', 'circumference', 'skinfold'];
    const index = categoryOrder.findIndex((category) => categories.includes(category));
    return index === -1 ? bodyMetricGroupOrder.length + 1 : index;
  }
...
<div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
  <div class="flex flex-col gap-3 border-b border-slate-100 bg-slate-50/50 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
    <div class="flex items-center">
      <div class="mr-3 h-6 w-1.5 rounded-full bg-violet-500"></div>
      <div>
        <h3 class="text-lg font-semibold text-slate-800">{m.body_measurements()}</h3>
        <p class="text-sm text-slate-500">{m.body_measurements_subtitle()}</p>
      </div>
    </div>
    <button
      type="button"
      onclick={openNewSession}
      class="inline-flex items-center gap-2 self-start rounded-lg bg-violet-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-violet-700 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-1 lg:self-auto"
    >
  ...
  {#if sessions.length === 0}
    <div class="px-6 py-14 text-center text-slate-400">
      ...
      <p class="text-sm font-medium">{m.no_body_measurements()}</p>
      <p class="mt-1 text-xs">{m.no_body_measurements_hint()}</p>
    </div>
  {:else}
    <div class="border-b border-slate-100 bg-gradient-to-br from-violet-50/60 via-white to-teal-50/40 px-6 py-5">
      <p class="text-xs font-semibold uppercase tracking-[0.22em] text-violet-700/80">{m.latest_measurements()}</p>
    ...
    <div>
      ...
      {#each orderedSessions as session (session.id)}
        ...
        <section id={`body-session-${session.id}`} class="border-t border-slate-100">
          <div class="flex flex-wrap items-center justify-between gap-3 px-6 py-4">
            ...
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                class="rounded-full border border-violet-200 bg-violet-50 px-3 py-1 text-xs font-semibold text-violet-700 transition-colors hover:bg-violet-100"
              >
                {m.edit_session()}
              </button>
              <form
                method="POST"
                action="?/deleteBodyMeasurement"
                use:enhance={(event) => {
